...
/// 高斯随机数(Box-Muller)
double gaussRand([double mean = 0, double std = 1, math.Random? random]) {
...
class ExpoFitResult {
  ...
  final double r2;
  final double center;
  final double centeredLogIntercept;
  const ExpoFitResult({
    required this.b,
    required this.r2,
    required this.center,
    required this.centeredLogIntercept,
  });

  /// The x=0 coefficient is optional: exp(log(a)) can overflow or underflow
  /// even when every prediction in the observed interval is ordinary-sized.
  double? get a {
    final value = math.exp(centeredLogIntercept - b * center);
    return value.isFinite && value > 0 ? value : null;
  }

  double evaluate(double x) {
    final value = math.exp(centeredLogIntercept + b * (x - center));
    if (!value.isFinite || value <= 0) {
      throw ArgumentError('指数预测超出浮点可表示范围，请检查预测区间或缩放响应');
    }
    return value;
  }
}
...
ExpoFitResult? exponentialFit(List<double> xs, List<double> ys) {
  ...
  return ExpoFitResult(
    b: fit.b,
    r2: fit.r2,
    center: fit.center,
    centeredLogIntercept: fit.centeredIntercept,
  );
}
...
/// Exact rational representation of a finite IEEE-754 double.
class Frac {
  final BigInt n;
  final BigInt d;
  ...
  static Frac _norm(BigInt n, BigInt d) {
    ...
    final divisor = n == BigInt.zero ? BigInt.one : n.gcd(d);
    return Frac._(n ~/ divisor, d ~/ divisor);
  }

  Frac operator +(Frac other) => _norm(n * other.d + other.n * d, d * other.d);
  Frac operator -(Frac other) => _norm(n * other.d - other.n * d, d * other.d);
  Frac operator *(Frac other) => _norm(n * other.n, d * other.d);
  Frac? div(Frac other) =>
      other.n == BigInt.zero ? null : _norm(n * other.d, d * other.n);
  bool get isZero => n == BigInt.zero;
  int cmp(Frac other) => (n * other.d).compareTo(other.n * d);

  static Frac fromDouble(double value) {
    if (!value.isFinite) {
      throw ArgumentError.value(value, 'value', '必须是有限数');
    }
    if (value == 0) return zero;
    final bits = (ByteData(8)..setFloat64(0, value)).getUint64(0);
    final sign = bits >>> 63;
    ...
    return _norm(numerator, denominator);
  }

  double toDouble() {
    int shiftFor(BigInt value) =>
        value.bitLength > 53 ? value.bitLength - 53 : 0;
    final numeratorShift = shiftFor(n.abs());
    final denominatorShift = shiftFor(d);
    final numerator = (n.abs() >> numeratorShift).toInt().toDouble();
    final denominator = (d >> denominatorShift).toInt().toDouble();
    final value =
        numerator /
        denominator *
        math.pow(2.0, numeratorShift - denominatorShift).toDouble();
    return n.isNegative ? -value : value;
  }
}

/// Exact intersection for two finite closed segments. Parallel and collinear
/// segments deliberately return null because their overlap is not one point.
({double x, double y})? exactSegIntersect(
  ...
  final p1 = Frac.fromDouble(p1x);
  final p1Y = Frac.fromDouble(p1y);
  final r = Frac.fromDouble(p2x) - p1;
  final rY = Frac.fromDouble(p2y) - p1Y;
  final q1 = Frac.fromDouble(q1x);
  final q1Y = Frac.fromDouble(q1y);
  final s = Frac.fromDouble(q2x) - q1;
  final sY = Frac.fromDouble(q2y) - q1Y;
  final determinant = r * sY - rY * s;
  if (determinant.isZero) return null;
  final qpx = q1 - p1;
  final qpy = q1Y - p1Y;
  final t = (qpx * sY - qpy * s).div(determinant);
  final u = (qpx * rY - qpy * r).div(determinant);
  if (t == null ||
      u == null ||
      t.cmp(Frac.zero) < 0 ||
      t.cmp(Frac.one) > 0 ||
...
double Function(double x, double y)? compileFormula(String src) {
  try {
    // Preserve the input node's documented shorthand: 3cos(t), 2sin(x).
    // Restrict insertion to known function calls so scientific notation such
    // as 1e-3 is never rewritten as multiplication by e.
